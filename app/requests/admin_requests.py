...
async def get_info_admin_order(index):
    async with async_session() as session:
        orders_d = await session.execute(select(Orders.id, Orders.data_time, Orders.address, Orders.order, Orders.total_cost, Orders.status).where(Orders.readiness == False, Orders.id == index))
    return orders_d.all()
...
async def get_inform_assembly_admin(index):
    async with async_session() as session:
        orders_d = await session.execute(select(Collecting_the_cake.id, Collecting_the_cake.event, Collecting_the_cake.image, Collecting_the_cake.description, Collecting_the_cake.data, Collecting_the_cake.address).where(Collecting_the_cake.readiness == False, Collecting_the_cake.id == index))
    return orders_d.all()
